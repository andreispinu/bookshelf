...
4. **How it works** (`id="how-it-works"`) — Two-column layout (stacked on mobile), each column in a stone-100 card. Sub-section 1 "Build your library" (BookOpen icon): 4 steps — Create account, Add books, Organise by category, Share your shelf. Sub-section 2 "Lend & borrow with friends" (ArrowLeftRight icon): 5 steps — Add friends, Browse their shelves, Request to borrow, Coordinate the handoff, Return smoothly. Driven by `LIBRARY_STEP_KEYS` and `LEND_STEP_KEYS` arrays in `page.tsx`; each key maps to `s_${key}_title` and `s_${key}_desc` translation keys in the `landing` namespace.
